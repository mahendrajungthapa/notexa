...
export default function NoteEditor({ content, onChange, editable = true, noteId, collabToken = '', aiEnabled = true, onRealtimeActiveChange }: NoteEditorProps) {
  ...
  const ToolButton = ({ onClick, active, children, title, className = '' }: any) => (
    <div className="relative group flex h-8 w-8 shrink-0 items-center justify-center">
      <button
        type="button"
        onClick={onClick}
        onMouseDown={(e) => e.preventDefault()}
        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-all duration-100 ${active ? 'bg-indigo-100 text-indigo-700 shadow-sm' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800 hover:scale-105'} ${className}`}
        aria-label={title}
      >
        {children}
      </button>
  ...
  return (
    <div className="flex flex-col flex-1 h-full min-h-0 tiptap-editor relative">
      ...
      {/* Toolbar */}
      {editable && (
        <div className="shrink-0 flex flex-wrap items-center gap-1 px-2 py-2 sm:px-4 sm:py-3 border-b border-slate-200/60 bg-white/80 backdrop-blur-sm z-10 sticky top-0 overflow-visible">
          {/* Text Style */}
          ...
          <div className="hidden xl:block flex-1 min-w-[10px]" />

          {/* AI Features Group */}
          {aiEnabled && (
            <div className="flex shrink-0 items-center gap-0.5 bg-gradient-to-r from-indigo-50/50 to-purple-50/50 p-1 rounded-xl border border-indigo-100/30">
          ...
          <div className="flex shrink-0 items-center gap-0.5 bg-gradient-to-r from-rose-50/50 to-pink-50/50 p-1 rounded-xl border border-rose-100/30 sm:ml-1.5 shadow-sm">
      ...
      {portalTarget && aiFeature === 'ask' && createPortal((
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[9999] p-3 sm:p-4 animate-in fade-in duration-200">
          <button
            ...
            className="fixed right-4 top-4 z-[10000] flex h-12 w-12 items-center justify-center rounded-full bg-white text-slate-800 shadow-2xl ring-1 ring-slate-200 transition hover:bg-slate-50 hover:text-slate-950 focus:outline-none focus:ring-4 focus:ring-indigo-500/30"
