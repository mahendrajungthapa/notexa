...
| AI | Admin-configured DeepSeek, OpenAI-compatible, or Gemini provider |
...
Users can generate summaries and run prompt-based tools from note content. The backend uses the configured DeepSeek, OpenAI-compatible, or Gemini provider without exposing provider API keys to clients.
